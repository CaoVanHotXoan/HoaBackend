...
let ChatbotService = ChatbotService_1 = class ChatbotService {
    constructor(databaseService, configService, ordersService, paymentService) {
    ...
    async processMessage(userId, message, sessionId, localCart) {
        if (!this.apiKey) {
            throw new common_1.InternalServerErrorException('AI Chatbot is currently unavailable due to missing API key.');
        }
        try {
            let newLocalCartToReturn = null;
          ...
          `;
                    ...
                    if (cartResult.recordset.length > 0) {
                        const cartLines = cartResult.recordset
                            .map((r) => `- ${r.Quantity}x ${r.ProductName} (ID sản phẩm=${r.ProductID}, Giá:${r.Price}đ)`)
                            .join('\n');
                        cartItemsData = cartResult.recordset;
                        subtotalData = cartResult.recordset.reduce((sum, r) => sum + r.Price * r.Quantity, 0);
                ...
                subtotalData = localCart.reduce((sum, r) => sum + r.Price * r.Quantity, 0);
                const shippingFee = 15000;
                const totalAmount = subtotalData + shippingFee;
        ...
        `;
                ...
                if (promoResult.recordset.length > 0) {
                    ...
                    const promos = availablePromos
                        .map((p) => `- Mã "${p.PromoCode}": ${p.Description} (Áp dụng cho đơn từ ${p.MinOrderValue} đ)`)
                        .join('\n');
            ...
            LIMIT 2
          `;
                    const trackingResult = await this.databaseService.query(trackingQuery, [{ name: 'UserID', value: userId }]);
...
Khách: "Thanh toán bằng VNPay, mã GIAM20K" (đã có địa chỉ ở câu trước) → Bạn: "[CHECKOUT_INTENT: {"address": "123 Lê Duẩn", "paymentMethod": "VNPay", "promoCode": "GIAM20K"}]"`;
        ...
        `;
            ...
            let cartMatch = null;
            const wasAskingConfirmation = lastBotResponse.includes('Giỏ hàng của bạn đang có') ||
            ...
            const wasAskingQuantity = lastBotResponse.includes('bạn muốn đặt bao nhiêu phần') ||
            ...
            const isUserConfirming = [
                ...
                'ok e',
                'ok shop',
            ].some((w) => message.toLowerCase().trim() === w ||
                message.toLowerCase().trim().startsWith(w));
            const lowerMsg = message.toLowerCase();
            if (wasAskingConfirmation &&
                isUserConfirming &&
                userId &&
                ...
                for (const line of lines) {
                    if (line.includes('muốn thêm') && line.includes('trong giỏ')) {
                        ...
                        if (nameMatch && qtyMatch) {
                            ...
                            const matchedProd = productsResult.recordset.find((p) => p.ProductName.toLowerCase() === pName.toLowerCase());
                            if (matchedProd) {
                                intentItems.push({
            ...
            if (isUserAskingReduceOrRemoveItem) {
                ...
                if (userId) {
                    try {
                        const currentCart = await this.databaseService.query('SELECT ci.CartItemID, ci.ProductID, ci.Quantity, p.ProductName FROM CartItems ci INNER JOIN Products p ON ci.ProductID = p.ProductID WHERE ci.UserID = @UserID', [{ name: 'UserID', value: userId }]);
                        if (currentCart.recordset.length === 0) {
                            ...
                            for (const item of currentCart.recordset) {
                                ...
                                const shortName = iName.split(' ')[0];
                                if (lowerMsg.includes(iName) ||
                            ...
                            if (targetItem) {
                                ...
                                else if (lowerMsg.includes('xóa hết') ||
                                    lowerMsg.includes('bỏ hết') ||
                                ...
                                if (targetItem.Quantity <= removeQty || removeQty <= 0) {
                                    await this.databaseService.query('DELETE FROM CartItems WHERE CartItemID = @CartItemID', [
                                        {
                                            name: 'CartItemID',
                                    ...
                                    await this.databaseService.query('UPDATE CartItems SET Quantity = @Quantity, UpdatedAt = CURRENT_TIMESTAMP WHERE CartItemID = @CartItemID', [
            ...
            const isUserAskingClear = !isLocalHandled &&
                [
                    'xóa giỏ',
                    'dọn giỏ',
                    'xóa hết giỏ',
            ...
            if (isUserAskingClear) {
                ...
                if (userId) {
                    try {
                        await this.databaseService.query('DELETE FROM CartItems WHERE UserID = @UserID', [{ name: 'UserID', value: userId }]);
                        isOrderPlaced = true;
                        responseText =
            ...
            const isUserAskingCheckout = !isLocalHandled &&
                ['thanh toán', 'đặt hàng', 'chốt đơn', 'mua hàng', 'tính tiền'].some((kw) => lowerMsg.includes(kw));
            if (isUserAskingCheckout && !userId) {
                isLocalHandled = true;
                responseText =
                    '❌ Vui lòng **Đăng nhập** để AI hỗ trợ bạn thanh toán và theo dõi đơn hàng nhé!';
            }
            const isOrderInquiry = !isLocalHandled &&
                [
                    'đơn hàng',
                    'đơn của mình',
                    'đơn của tôi',
            ...
            if (!isLocalHandled && userId && productsResult.recordset.length > 0) {
                ...
                if (!isExcluded) {
                    ...
                    if (foundTerms.length > 0) {
                        const ambiguousList = [];
                        const wasJustAskedToClarify = lastBotResponse.includes('Trong thực đơn có nhiều món') ||
                        ...
                        if (ambiguousList.length > 0) {
                            ...
                            if (intentItems.length > 0) {
                                for (const item of intentItems) {
                                    ...
                                    if (checkProd) {
                                        const checkCart = await this.databaseService.query('SELECT CartItemID, Quantity FROM CartItems WHERE UserID = @UserID AND ProductID = @ProductID', [
                                            { name: 'UserID', value: userId },
                                            { name: 'ProductID', value: item.id },
                                        ]);
                                        const currentQty = checkCart.recordset.length > 0
                                        ...
                                        if (currentQty + item.qty > checkProd.Inventory) {
                                            ...
                                            if (checkCart.recordset.length > 0) {
                                                ...
                                                await this.databaseService.query('INSERT INTO CartItems (UserID, ProductID, Quantity, UpdatedAt) VALUES (@UserID, @ProductID, @Quantity, CURRENT_TIMESTAMP)', [
                                                    { name: 'UserID', value: userId },
                                                    {
                                                        name: 'ProductID',
                                                        value: item.id,
            ...
            if (!isLocalHandled &&
                ...
                const callGroqAPI = async (retryCount = 0) => {
                    ...
                    try {
                        ...
                        if (parsedData && Array.isArray(parsedData.items)) {
                            ...
                            if (!hasQtyWord) {
                                ...
                                intentItems = parsedData.items.map((i) => ({
                                    id: parseInt(i.id),
                ...
                const clearCartMatch = typeof responseText === 'string' &&
            ...
            if (intentItems.length > 0 && userId) {
                try {
                    ...
                    for (const item of intentItems) {
                        const prodCheck = await this.databaseService.query('SELECT ProductName, Inventory FROM Products WHERE ProductID = @ProductID AND IsActive = true', [{ name: 'ProductID', value: item.id }]);
                        if (prodCheck.recordset.length === 0)
                            continue;
                        const product = prodCheck.recordset[0];
                        const checkCart = await this.databaseService.query('SELECT ci.CartItemID, ci.Quantity, p.ProductName FROM CartItems ci INNER JOIN Products p ON ci.ProductID = p.ProductID WHERE ci.UserID = @UserID AND ci.ProductID = @ProductID', [
                            { name: 'UserID', value: userId },
                            { name: 'ProductID', value: item.id },
                        ]);
                        const currentQty = checkCart.recordset.length > 0
                    ...
                    if (outOfStockItems.length > 0 &&
                        ...
                        const confirmLines = duplicateItems
                            .map((d) => `• **${d.name}** (trong giỏ đang có **${d.currentQty}** phần. Bạn có chắc muốn thêm **${d.addQty}** nữa không?)`)
                            .join('\n');
                        let outOfStockMsg = '';
                        if (outOfStockItems.length > 0) {
                        ...
                        for (const item of itemsToProcess) {
                            ...
                            if (checkCart.recordset.length > 0) {
                                ...
                                await this.databaseService.query('UPDATE CartItems SET Quantity = @Quantity, UpdatedAt = CURRENT_TIMESTAMP WHERE UserID = @UserID AND ProductID = @ProductID', [
                                    { name: 'Quantity', value: newQty },
                                    { name: 'UserID', value: userId },
                                    { name: 'ProductID', value: item.id },
                                ]);
                        ...
                        isOrderPlaced = true;
                        const updatedCartResult = await this.databaseService.query('SELECT SUM(c.Quantity * p.Price) as Subtotal FROM CartItems c INNER JOIN Products p ON c.ProductID = p.ProductID WHERE c.UserID = @UserID', [{ name: 'UserID', value: userId }]);
            ...
            else if ((intentItems.length > 0 || cartMatch) &&
                !userId &&
                !responseText.startsWith('❌ Rất tiếc')) {
                try {
                    ...
                    const addedNames = [];
                    for (const item of intentItems) {
                        const prodCheck = await this.databaseService.query('SELECT ProductName, Price, ImageURL, Inventory FROM Products WHERE ProductID = @ProductID AND IsActive = true', [{ name: 'ProductID', value: item.id }]);
                        if (prodCheck.recordset.length === 0)
                            continue;
                        const product = prodCheck.recordset[0];
                        const existingIndex = tempLocalCart.findIndex((i) => parseInt(i.ProductID) === item.id);
            ...
            if (checkoutMatch && userId) {
                try {
                    ...
                    if (intentData.address) {
                        const cartCheck = await this.databaseService.query('SELECT COUNT(*) as count FROM CartItems WHERE UserID = @UserID', [{ name: 'UserID', value: userId }]);
                        if (cartCheck.recordset[0].count === 0) {
                            ...
                            const appliedPromo = intentData.promoCode
                                ...
                                : null;
                            const orderResult = await this.ordersService.createOrder(userId, intentData.address, null, null, intentData.paymentMethod || 'Tiền mặt', appliedPromo, 15000);
                            isOrderPlaced = true;
                            ...
                            if (isVNPay && orderResult && orderResult.OrderID) {
                                const orderData = await this.databaseService.query('SELECT FinalAmount FROM Orders WHERE OrderID = @OrderID', [{ name: 'OrderID', value: orderResult.OrderID }]);
                                const finalAmount = orderData.recordset[0]?.FinalAmount || 0;
                                const bankId = this.configService.get('VIETQR_BANK_ID') || 'namabank';
                                const accountNo = this.configService.get('VIETQR_ACCOUNT_NO') || '0824781046';
                                const accountName = this.configService.get('VIETQR_ACCOUNT_NAME') || 'NGUYEN THI THU HOA';
                                ...
                                let successMsg = '✅ **Đặt hàng thành công!** Đơn sẽ được giao tới: **' +
                                    intentData.address +
                                    '**';
                                if (appliedPromo) {
            ...
            if (typeof responseText === 'string') {
                if (responseText.includes('[CHECKOUT_INTENT'))
                    hasCheckoutIntent = true;
                responseText = responseText
                    .replace(/\s*\(\s*ID=\d+\s*\)/gi, '')
                    .replace(/\s*ID=\d+\s*\|/gi, '')
                    .replace(/\[[A-Z_]+_INTENT:?.*?\]/gs, '')
                    .trim();
            }
            if (typeof responseText === 'string') {
                const recommendedFoods = [];
                if (!responseText.includes('Giỏ hàng hiện tại đang có') &&
            ...
            if (typeof responseText === 'string' &&
                ...
                const hasMultipleFoods = richContent &&
                ...
                if (availablePromos && availablePromos.length > 0) {
                    ...
                    if (!message.toLowerCase().includes('xem danh sách')) {
                        ...
                        if (mentionedPromos.length === 0)
                            mentionedPromos = availablePromos;
                    }
            ...
            if (!richContent || richContent.type !== 'promotions') {
                ...
                if (lowerRes.includes('kiểm tra lại') ||
    ...
    async getLogs() {
      ...
      `;
            ...
            throw new common_1.InternalServerErrorException('Failed to fetch chatbot logs');
        }
    }
    async generatePromotionContent(data) {
        if (!this.apiKey)
            throw new common_1.InternalServerErrorException('Missing API key.');
...
Yêu cầu: Viết thành 1 đoạn văn ngắn (dưới 50 chữ), sử dụng những tính từ miêu tả sự tươi ngon, đậm đà, nóng hổi. Trả về đúng nội dung mô tả, không cần giải thích hay thêm thắt.`;
    ...
    async callGroq(prompt, expectJson = false) {
        const res = await fetch('https://api.deepseek.com/chat/completions', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
    ...
    async setWebsiteAnnouncement(content, productId) {
